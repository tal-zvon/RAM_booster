...
#Make sure the OS this is being run on is supported (Ubuntu 10.04 or 10.10)
Check_OS()
{
OS_Check=`cat /etc/issue | grep -o '[0-9][0-9]*\.[0-9][0-9]*'`

if [[ "$OS_Check" != "10.04" ]] && [[ "$OS_Check" != "10.10" ]]
then
	echo "This script was written to work with Ubuntu 10.04 or Ubuntu 10.10. You are running `cat /etc/issue | egrep -o '[a-Z]+[ ][0-9]+\.[0-9]+\.*[0-9]*'`. This means the script has NOT been tested for your OS. Run this at your own risk."  | fmt -w `tput cols`
	echo
...
echo "Installing live-initramfs..."
sudo apt-get -y --force-yes install live-initramfs 2>/dev/null >/dev/null

if [[ "$?" != 0 ]]
then
	#Attempt to download and install the .deb file
	#At the time of writing, Ubuntu 10.10 does not have
	#live-initramfs in its repos making this step important
	echo "live-initramfs was not found in the repo. Attempting to download .deb"
	echo "Installing dependencies for live-initramfs: busybox, user-setup..."
	sudo apt-get -y --force-yes install busybox user-setup 2>/dev/null >/dev/null || { echo "Failed to install dependencies for live-initrams. You'll have to download and install live-initramfs and its dependencies manually"; exit 1; }
	echo "Downloading and installing live-initramfs .deb..."
	(cd /tmp; wget http://launchpadlibrarian.net/38889169/live-initramfs_1.173.1-1_all.deb && sudo dpkg -i live-initramfs_1.173.1-1_all.deb && sudo rm -f live-initramfs_1.173.1-1_all.deb) 2>/dev/null >/dev/null ||
	{ echo "live-initramfs failed to install. You'll have to download and install it manually... Try: https://launchpad.net/ubuntu/maverick/+package/live-initramfs" | fmt -w `tput cols`; exit 1; }
fi
...
case "$EX_Code" in
	...
	24)
		...
		echo -e "\nSome files vanished since Filesystem duplication began:\n"
		grep 'vanished' /tmp/fs_sync
