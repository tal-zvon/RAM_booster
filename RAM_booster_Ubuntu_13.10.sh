...
#Make sure the OS this is being run on is supported (Ubuntu 13.10)
Check_OS()
{
...
if [[ "$OS_Check" != "13.10" ]]
then
	echo "This script was written to work with Ubuntu 13.10. You are running `cat /etc/issue | egrep -o '[a-Z]+[ ][0-9]+\.[0-9]+\.*[0-9]*'`. This means the script has NOT been tested for your OS. Run this at your own risk."  | fmt -w `tput cols`
	echo
...
sudo sed -i 's#\(echo " [*] Copying $MODULETORAMFILE to RAM" 1>/dev/console\)#\1\
				...
				echo 1>/dev/console#g' 9990-toram-todisk.sh 2>/dev/null
